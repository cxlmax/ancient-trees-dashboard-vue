import { defineComponent as u, watch as i, createElementBlock as b, openBlock as v, createElementVNode as E, unref as _ } from "vue";
import { cloneDeep as r, isEqual as O } from "lodash";
import { useChart as g } from "../../commons/core/useChart.mjs";
import { cloneSeries as D } from "../../commons/utils/cloneSeries.mjs";
import { parseScatterData as k } from "../../commons/utils/parseScatterData.mjs";
import { SHJDatasourceV2 as w } from "../../commons/plugins/datasource/index.mjs";
import { SHJParseEvent as n } from "../../commons/plugins/event/index.mjs";
const x = { class: "zerov-widget" }, S = ["id"], C = u({ name: "zv-chart-scatter-bubble" }), y = /* @__PURE__ */ u({
  ...C,
  props: {
    basicOption: {},
    sources: {},
    useEvents: {},
    uuid: {}
  },
  emits: [
    "chart-click",
    "chart-dblclick",
    "chart-mousedown",
    "chart-mouseover",
    "chart-mouseout",
    "chart-contextmenu"
  ],
  setup(m, { expose: p, emit: l }) {
    const e = m, d = l, {
      id: h,
      setOption: o,
      chart: f,
      renderChart: c,
      renderData: a
    } = g({
      basicOption: r(e.basicOption),
      useEvents: r(e.useEvents),
      customRenderData: (s) => {
        s = r(s), w.parse({
          tId: e.uuid,
          sources: e.sources,
          callback: ({ data: t }) => {
            try {
              n.parseEvents(e.useEvents, "dataListener", t), s.dataset = k(r(t)), s.series = D(t, s.series), o(s);
            } catch {
              n.parseEvents(e.useEvents, "dataListener", null), o(s);
            }
          }
        });
      },
      emit: d
    });
    return i(() => e.sources, () => {
      e.sources && e.sources.length > 0 && a(e.basicOption);
    }, {
      deep: !0
    }), i(() => r(e.basicOption), (s, t) => {
      O(s, t) || c(s);
    }, {
      deep: !0
    }), p({
      getEchartsInstance: () => f.value,
      refresh: () => {
        e.sources && e.sources.length > 0 && c(e.basicOption), a(e.basicOption);
      },
      refreshView: () => {
        e.sources && e.sources.length > 0 && c(e.basicOption);
      },
      refreshData: () => {
        a(e.basicOption);
      }
    }), (s, t) => (v(), b("div", x, [
      E("div", {
        id: _(h),
        class: "widget"
      }, null, 8, S)
    ]));
  }
});
export {
  y as default
};
